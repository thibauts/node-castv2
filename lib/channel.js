...
function Channel(bus, sourceId, namespace, encoding) {
  ...
  function onmessage(sourceId, destinationId, namespace, data) {
    ...
    self.emit('message', sourceId, decode(data, self.encoding), destinationId === '*');
  }
...
Channel.prototype.send = function(destinationId, data) {
  this.bus.send(
    this.sourceId,
    destinationId,
    this.namespace,
