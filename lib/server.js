  ...
  function onconnect(socket) {
    ...
    function onpacket(buf) {
      ...
      debug(
        ...
        message.namespace,
        (message.payload_type === 'BINARY' || message.payload_type === 1)
          ? util.inspect(message.payload_binary)
          : message.payload_utf8
      );

      if(message.protocol_version !== 'CASTV2_1_0' && message.protocol_version !== 0) {
...
Server.prototype.send = function(clientId, sourceId, destinationId, namespace, data) {
  ...
  debug(
    ...
    message.destination_id,
    message.namespace,
    (message.payload_type === 'BINARY' || message.payload_type === 1) // BINARY
      ? util.inspect(message.payload_binary)
      : message.payload_utf8
  );
